...
describe 'compiled component network-loadbalancer' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/tcp_listener.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/tcp_listener/network-loadbalancer.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "mysqlTargetGroup" do
      ...
      it "to have property Targets" do
          expect(resource["Properties"]["Targets"]).to eq([{"Id"=>"10.1.2.16/32", "Port"=>3306}])
      end
