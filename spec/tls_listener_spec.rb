...
describe 'compiled component network-loadbalancer' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/tls_listener.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/tls_listener/network-loadbalancer.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "appTargetGroup" do
      let(:resource) { template["Resources"]["appTargetGroup"] }
    ...
    context "httpsListener" do
      let(:resource) { template["Resources"]["httpsListener"] }
      ...
      it "to have property Protocol" do
          expect(resource["Properties"]["Protocol"]).to eq("TLS")
      end
      ...
      it "to have property Certificates" do
          expect(resource["Properties"]["Certificates"]).to eq([{"CertificateArn"=>{"Ref"=>"SslCertId"}}])
      end
      
      it "to have property DefaultActions" do
          expect(resource["Properties"]["DefaultActions"]).to eq([{"TargetGroupArn"=>{"Ref"=>"appTargetGroup"}, "Type"=>"forward"}])
      end
      
    end
    
    context "appLoadBalancerRecord" do
      let(:resource) { template["Resources"]["appLoadBalancerRecord"] }
      ...
      it "to have property Name" do
          expect(resource["Properties"]["Name"]).to eq({"Fn::Sub"=>"app.${EnvironmentName}.${DnsDomain}."})
      end
