...
describe 'compiled component network-loadbalancer' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/http_healthcheck.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/http_healthcheck/network-loadbalancer.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "mysqlTargetGroup" do
      ...
      it "to have property HealthCheckPort" do
          expect(resource["Properties"]["HealthCheckPort"]).to eq(8080)
      end
      
      it "to have property HealthCheckProtocol" do
          expect(resource["Properties"]["HealthCheckProtocol"]).to eq("HTTP")
      end
      
      it "to have property HealthCheckPath" do
          expect(resource["Properties"]["HealthCheckPath"]).to eq("/health")
      end
